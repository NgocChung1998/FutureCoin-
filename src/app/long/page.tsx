...
import { ArrowLeft, TrendingUp, Radio } from "lucide-react";

import { getReportData } from "@/lib/data";
import { CoinCard } from "@/components/CoinCard";
import { Navigation } from "@/components/Navigation";

export default async function LongPage() {
  const { coins } = await getReportData();
  const longCoins = coins.filter((coin) => coin.preferredAction?.includes("LONG"));
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-[#020617] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-900/20 via-[#020617] to-[#020617] text-slate-100 p-4 sm:p-8">
      ...
      <div className="w-full max-w-[1400px] rounded-3xl border border-white/10 bg-[#0b1221] shadow-[0_0_100px_-20px_rgba(16,185,129,0.2)] backdrop-blur-2xl overflow-hidden flex flex-col min-h-[85vh]">
        ...
        <header className="flex h-16 items-center justify-between border-b border-white/5 bg-white/[0.02] px-8">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 rounded-full bg-emerald-500/10 px-3 py-1 border border-emerald-500/20">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-500">LONG Recommendations</span>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <Navigation />
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="flex items-center gap-3">
                   ...
                   <h1 className="text-5xl font-black tracking-tighter text-white">LONG</h1>
                </div>
                <p className="mt-2 text-sm font-medium text-slate-400 uppercase tracking-wide">
                  Tổng số coin: <span className="text-white text-2xl">{longCoins.length}</span> khuyến nghị
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="space-y-6">
            {longCoins.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                Không có coin nào được khuyến nghị LONG tại thời điểm này.
              </div>
            ) : (
